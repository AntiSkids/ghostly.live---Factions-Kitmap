...
public class CombatLogger {
    ...
    @Getter
    private final long deathbanDuration;
    @Getter
    ...
    public CombatLogger(Player player) {
        ...
        this.entity.setMetadata("CombatLogger", new FixedMetadataValue(FactionsPlugin.getInstance(), this));
        loggers.put(this, System.currentTimeMillis());
    }

    public static CombatLogger getByEntity(LivingEntity entity) {
        return loggers.keySet().stream().filter(logger -> logger.getEntity().getEntityId() == entity.getEntityId()).findFirst().orElse(null);
    }
