package live.ghostly.hcfactions.crowbar;
...
public class Crowbar {
    ...
    private void setup() {
        ...
        try {
            spawner = Integer.parseInt(ChatColor.stripColor(itemStack.getItemMeta().getLore().get(spawnerIndex)).replaceAll("[^0-9]", "").replace(" ", ""));
        } catch (Exception ignored) {
            ignored.printStackTrace();
            spawner = 0;
        }
        try {
            portal = Integer.parseInt(ChatColor.stripColor(itemStack.getItemMeta().getLore().get(portalIndex)).replaceAll("[^0-9]", "").replace(" ", ""));
        } catch (Exception ignored) {
            ignored.printStackTrace();
            portal = 0;
        }
