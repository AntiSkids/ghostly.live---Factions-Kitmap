...
public class ScheduleHandler {
    ...
    public static List<Schedule> kothschedules = new ArrayList<>();
    public static List<Schedule> citadelSchedules = new ArrayList<>();

    public static void setSchedules(EventManager manager, ConfigFile config) {

        if (config.getFile().getName().contains("koth")) {
            ...
            if (config.getConfiguration().contains("schedule-monday")) {
                readSchedule(manager, config, "monday", 0, calendarMillis);
                Bukkit.getLogger().info("[HCF] (CITADEL): Loaded schedule for monday.");
            }

            if (config.getConfiguration().contains("schedule-tuesday")) {
                readSchedule(manager, config, "tuesday", 1, calendarMillis);
                Bukkit.getLogger().info("[HCF] (CITADEL): Loaded schedule for tuesday.");
            }

            if (config.getConfiguration().contains("schedule-wednesday")) {
                readSchedule(manager, config, "wednesday", 2, calendarMillis);
                Bukkit.getLogger().info("[HCF] (CITADEL): Loaded schedule for wednesday.");
            }

            if (config.getConfiguration().contains("schedule-thursday")) {
                readSchedule(manager, config, "thursday", 3, calendarMillis);
                Bukkit.getLogger().info("[HCF] (CITADEL): Loaded schedule for thursday.");
            }

            if (config.getConfiguration().contains("schedule-friday")) {
                readSchedule(manager, config, "friday", 4, calendarMillis);
                Bukkit.getLogger().info("[HCF] (CITADEL): Loaded schedule for friday.");
            }

            if (config.getConfiguration().contains("schedule-saturday")) {
                readSchedule(manager, config, "saturday", 5, calendarMillis);
                Bukkit.getLogger().info("[HCF] (CITADEL): Loaded schedule for saturday.");
            }

            if (config.getConfiguration().contains("schedule-sunday")) {
                readSchedule(manager, config, "sunday", 6, calendarMillis);
                Bukkit.getLogger().info("[HCF] (CITADEL): Loaded schedule for sunday.");
            }
        }
        ...
        if (config.getFile().getName().contains("koth")) {
            try {
                ...
                for (int i = 0; i < events.length; i++) {
                    ...
                    kothschedules.add(new Schedule(eventTime, runTime, area, StringUtils.capitalize(day) + " " + time));
                }
            ...
            try {
                ...
                for (int i = 0; i < events.length; i++) {
                    ...
                    citadelSchedules.add(new Schedule(eventTime, runTime, area, StringUtils.capitalize(day) + " " + time));
                }
    ...
    public static Schedule getNextEvent(boolean citadel) {

        if (!citadel) {
            Schedule schedule = null;

            for (Schedule sched : kothschedules) {
            ...
            for (Schedule sched : citadelSchedules) {
    ...
    public static void runSchedule() {
        for (Schedule schedule : kothschedules) {
            schedule.runEvent();
        }
        for (Schedule schedule : citadelSchedules) {
            schedule.runEvent();
        }
    }
}
