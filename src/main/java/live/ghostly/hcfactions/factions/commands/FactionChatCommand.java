...
import live.ghostly.hcfactions.profile.ProfileChatType;
...
public class FactionChatCommand extends FactionCommand implements Listener {
    ...
    public void onCommand(CommandArgs command) {
        ...
        List<String> chatTypes = new ArrayList<>(Arrays.asList(
                "a", "ally", "f", "faction", "p", "public", "o", "officer"
        ));
        ...
        if (arg.equalsIgnoreCase("o") || arg.equalsIgnoreCase("officer")) {
            if (!profile.getFaction().getOfficers().contains(player.getUniqueId()) &&
                    !profile.getFaction().getCoLeaders().contains(player.getUniqueId()) &&
                    !profile.getFaction().getLeader().equals(player.getUniqueId())) {
                player.sendMessage(langConfig.getString("ERROR.NOT_OFFICER").replace("%PLAYER%", player.getName()));
                return;
            }
            setChatType(player, ProfileChatType.OFFICER);
        }
    }
    ...
    private void setChatType(Player player, ProfileChatType type) {
        ...
        switch (type) {
            ...
            case OFFICER: {
                player.sendMessage(langConfig.getString(ROOT + "OFFICER"));
                break;
            }
        }
    }
    ...
    public void onAsyncPlayerChat(AsyncPlayerChatEvent event) {
        ...
        if (inFactionChat || profile.getChatType() == ProfileChatType.ALLY || profile.getChatType() == ProfileChatType.OFFICER) {
            event.setCancelled(true);
            ...
            if (inFactionChat) {
            ...
            } else if (profile.getChatType() == ProfileChatType.ALLY) {
                ...
                playerFaction.sendMessageOfficer(langConfig.getString("ANNOUNCEMENTS.FACTION.PLAYER_OFFICER_CHAT").replace("%PLAYER%", player.getName()).replace("%MESSAGE%", event.getMessage()).replace("%FACTION%", playerFaction.getName()));
            }
        }
    }
}
