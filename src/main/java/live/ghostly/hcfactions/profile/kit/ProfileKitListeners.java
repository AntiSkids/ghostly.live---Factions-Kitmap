                ...
                for (Player player : PlayerUtility.getOnlinePlayers()) {
                            ...
                            for (PotionEffect effect : new HashSet<>(player.getActivePotionEffects())) {
                                if (effect.getType().equals(cachedEffect.getType())) {
                                    if (effect.getDuration() <= 20) {
                                        ...
                                        Bukkit.getServer().getScheduler().runTask(FactionsPlugin.getInstance(), () -> player.addPotionEffect(cachedEffect, true));
                    ...
                    if (profile.getKit() == null) {
                        ...
                        if (kitWarmup != null) {
                            if (!(kitWarmup.getKit().isWearingArmor(player))) {
                                ...
                                if (kitWarmup.getTimeLeft().equalsIgnoreCase("0.1") || kitWarmup.getTimeLeft().equalsIgnoreCase("0.0") || kitWarmup.getTimeLeft().contains("-")) {
                                    profile.setKitWarmup(null);
                    ...
                    if (!(profile.getKit().isWearingArmor(player))) {
                        ...
                        if (profile.getKit() == ProfileKit.MINER) {
                            ...
                            if (diamondsMined >= 100) {
                                effects.add(new ProfileKitPotionEffect(PotionEffectType.FAST_DIGGING, 3));
                            }

                            if (diamondsMined >= 200) {
                                effects.add(new ProfileKitPotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 1));
                            }

                            if (diamondsMined >= 300) {
                                effects.add(new ProfileKitPotionEffect(PotionEffectType.SPEED, 1));
                            }

                            if (diamondsMined >= 400) {
                        ...
                        for (ProfileKitPotionEffect effect : effects) {
                            ...
                            if (add) {
                                ...
                                Bukkit.getServer().getScheduler().runTask(FactionsPlugin.getInstance(), () -> player.addPotionEffect(new PotionEffect(effect.getType(), 20 * 20, effect.getLevel() - 1, true), true));
                            }
                        }
                    }
