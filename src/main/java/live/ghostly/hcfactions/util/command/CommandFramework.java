        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (command.inGameOnly() && sender instanceof Player && command.inFactionOnly()) {
                    ...
                    if (command.isCoLeaderOnly() && !faction.getLeader().equals(profile.getUuid()) && !faction.getCoLeaders().contains(profile.getUuid())) {
                        sender.sendMessage(plugin.getLanguageConfig().getString("ERROR.NOT_COLEADER_OR_LEADER"));
                        return true;
                    }
